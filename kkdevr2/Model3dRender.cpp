...
void SetColorMat( Material *RobotMat,double c1_h, double c1_s, double c1_v)
{
...
int main(int argc, char *argv[])
{
 // ui_loop(argc, argv, "models");
  ...
  graphLib.gl_ctx.glXSaveRenderImg(ScreenPic);
  ScreenPic.Save("imgok1.bmp");


  //ScreenPic.Save("imgok2.bmp");
  //Mat image, imageCopy;
  int i,j;
  Mat img(1600, 1600, CV_8UC3, Scalar(0, 0, 0));
  int count(0);
  while(1)
  { 
     count = count%360;
     SetColorMat(world_model.StlElements[0]->MatNode,
	 count, 1.0, 1.0);
	 count++;
     world_model.MyViewer->Display0();
     graphLib.gl_ctx.glXSaveRenderImg(ScreenPic);
     ScreenPic.Save("test.bmp");
     Loopi(ScreenPic.Height)
	 {
		unsigned char* lineptr = img.ptr<uchar>(ScreenPic.Height-1-i);
		memcpy(lineptr, &ScreenPic.Buffer[ScreenPic.LineWidth*i], ScreenPic.LineWidth);
		 // Loopj(ScreenPic.Width)
     // {
       //Vec3b intensity = img.at<Vec3b>(j, i);
	   //C24PixVal Pix = get_pix_color(ScreenPic,i, j);
       //intensity.val[0] = *Pix.r;
       //intensity.val[1] = *Pix.g;
       //intensity.val[2] = *Pix.b ;
      //}
	 }
    imshow("out", img);
