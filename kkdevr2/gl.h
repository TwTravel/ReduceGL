...
public:
 ...
 // void *opaque;
  ...
  void inline glXSaveRenderImg(C24BitMap&CPic );
};// GLContext;
...
void inline GLContext::glInit(int xsize,int ysize)
{
  ...
  /* default state */
  this->current_color.vX=1.0;
  this->current_color.vY=1.0;
  this->current_color.vZ=1.0;
  this->current_color.vW=1.0;
  this->longcurrent_color[0] = 65535;
  this->longcurrent_color[1] = 65535;
  this->longcurrent_color[2] = 65535;

  this->current_normal.vX=1.0;
  this->current_normal.vY=0.0;
  this->current_normal.vZ=0.0;
  this->current_normal.vW=0.0;

  this->current_edge_flag=1;
  
  this->current_tex_coord.vX=0;
  this->current_tex_coord.vY=0;
  this->current_tex_coord.vZ=0;
  this->current_tex_coord.vW=1;
...
void inline GLContext::glXSaveRenderImg(C24BitMap&CPic )
{
...
inline void GraphDrawLib::gl_eval_viewport(GLContext * c)
{
    ...
    v->trans.vX = ((v->xsize - 0.5) / 2.0) + v->xmin;
    v->trans.vY = ((v->ysize - 0.5) / 2.0) + v->ymin;
    v->trans.vZ = ((zsize - 0.5) / 2.0) + ((1 << ZB_POINT_Z_FRAC_BITS)) / 2;

    v->scale.vX = (v->xsize - 0.5) / 2.0;
    v->scale.vY = -(v->ysize - 0.5) / 2.0;
    v->scale.vZ = -((zsize - 0.5) / 2.0);
}
...
{
  GLContext *c=&gl_ctx;; 
  c->current_normal.vX = x;
  c->current_normal.vY = y;
  c->current_normal.vZ = z;
  c->current_normal.vW = 0;
}
...
inline void GraphDrawLib::glColor4f(float r,float g,float b,float a)
{
  GLContext *c=&gl_ctx;; 
    ...
    c->current_color.vX = r;
    c->current_color.vY = g;
    c->current_color.vZ = b;
    c->current_color.vW = a;
...
{
    GLContext *c=&gl_ctx;; 
    c->current_tex_coord.vX =  s;
    c->current_tex_coord.vY =  t;
    c->current_tex_coord.vZ =  r;
    c->current_tex_coord.vW =  q;
}
  ...
  case GL_POSITION:
    {
      ...
      if (l->position.v[3] == 0) {
        l->norm_position.vX=pos.vX;
        l->norm_position.vY=pos.vY;
        l->norm_position.vZ=pos.vZ;
...
void inline gl_transform_to_viewport(GLContext *c,GLVertex *v)
{
  ...
  /* coordinates */
  winv=1.0/v->pc.vW;
  v->zp.x= (int) ( v->pc.vX * winv * c->viewport.scale.vX 
                   + c->viewport.trans.vX );
  v->zp.y= (int) ( v->pc.vY * winv * c->viewport.scale.vY 
                   + c->viewport.trans.vY );
  v->zp.z= (int) ( v->pc.vZ * winv * c->viewport.scale.vZ 
                   + c->viewport.trans.vZ );
  /* color */
  if (c->lighting_enabled) {
  ...
  if (c->texture_2d_enabled) {
    v->zp.s=(int)(v->tex_coord.vX * (ZB_POINT_S_MAX - ZB_POINT_S_MIN) 
                  + ZB_POINT_S_MIN);
    v->zp.t=(int)(v->tex_coord.vY * (ZB_POINT_T_MAX - ZB_POINT_T_MIN) 
                  + ZB_POINT_T_MIN);
  }
}
 ...
 void inline gl_vertex_transform(GLContext * c, GLVertex * v)
{
	...
	v->ec.vX = (v->coord.vX * m[0] + v->coord.vY * m[1] +
		   v->coord.vZ * m[2] + m[3]);
	v->ec.vY = (v->coord.vX * m[4] + v->coord.vY * m[5] +
		   v->coord.vZ * m[6] + m[7]);
	v->ec.vZ = (v->coord.vX * m[8] + v->coord.vY * m[9] +
		   v->coord.vZ * m[10] + m[11]);
	v->ec.vW = (v->coord.vX * m[12] + v->coord.vY * m[13] +
		   v->coord.vZ * m[14] + m[15]);

	/* projection coordinates */
	m = //&c->matrix_buffer_ptr[1];
	 &(c->TkGLMat[KH_PROJECTION].get_current_matrix()->m[0][0]);
	v->pc.vX = (v->ec.vX * m[0] + v->ec.vY * m[1] +
		   v->ec.vZ * m[2] + v->ec.vW * m[3]);
	v->pc.vY = (v->ec.vX * m[4] + v->ec.vY * m[5] +
		   v->ec.vZ * m[6] + v->ec.vW * m[7]);
	v->pc.vZ = (v->ec.vX * m[8] + v->ec.vY * m[9] +
		   v->ec.vZ * m[10] + v->ec.vW * m[11]);
	v->pc.vW = (v->ec.vX * m[12] + v->ec.vY * m[13] +
		   v->ec.vZ * m[14] + v->ec.vW * m[15]);

	m = &c->matrix_model_view_inv.m[0][0];
	n = &c->current_normal;

	v->normal.vX = (n->vX * m[0] + n->vY * m[1] + n->vZ * m[2]);
	v->normal.vY = (n->vX * m[4] + n->vY * m[5] + n->vZ * m[6]);
	v->normal.vZ = (n->vX * m[8] + n->vY * m[9] + n->vZ * m[10]);
    ...
    v->clip_code = gl_clipcode(v->pc.vX, v->pc.vY, v->pc.vZ, v->pc.vW);
}
			     ...
			     GLVertex *q,GLVertex *p0,GLVertex *p1,float t)
{
  ...
  if (c->texture_2d_enabled) {
    q->tex_coord.vX=p0->tex_coord.vX + (p1->tex_coord.vX-p0->tex_coord.vX)*t;
    q->tex_coord.vY=p0->tex_coord.vY + (p1->tex_coord.vY-p0->tex_coord.vY)*t;
  }

  q->clip_code=gl_clipcode(q->pc.vX,q->pc.vY,q->pc.vZ,q->pc.vW);
  if (q->clip_code==0)
...
void  inline  GraphDrawLib::gl_shade_vertex(GLContext *c,GLVertex *v)
{
  ...
  for(l=c->first_light;l!=NULL;l=l->next) {
    ...
    if (l->position.v[3] == 0) {
      ...
      d.vX=l->position.v[0]-v->ec.v[0];
      d.vY=l->position.v[1]-v->ec.v[1];
      d.vZ=l->position.v[2]-v->ec.v[2];
      dist=sqrt(d.vX*d.vX+d.vY*d.vY+d.vZ*d.vZ);
      if (dist>1E-3) {
        tmp=1/dist;
        d.vX*=tmp;
        d.vY*=tmp;
    ...
    dot=d.vX*n.vX+d.vY*n.vY+d.vZ*n.vZ;
    ...
    if (dot>0) {
      ...
      if (l->spot_cutoff != 180) {
        dot_spot=-(d.vX*l->norm_spot_direction.v[0]+
                   d.vY*l->norm_spot_direction.v[1]+
                   d.vZ*l->norm_spot_direction.v[2]);
      ...
      dot_spec=n.vX*s.vX+n.vY*s.vY+n.vZ*s.vZ;
      if (twoside && dot_spec < 0) dot_spec = -dot_spec;
      if (dot_spec>0) {
        GLSpecBuf *specbuf;
        int idx;
        tmp=sqrt(s.vX*s.vX+s.vY*s.vY+s.vZ*s.vZ);
        if (tmp > 1E-3) {
